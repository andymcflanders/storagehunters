<script lang="ts">
	...
	async function decide(decision: TriageDecision) {
		...
		try {
			...
			// Reset transform state before the next card mounts so it
			// doesn't briefly render off-screen.
			dx = 0;
			dy = 0;
			exitDirection = null;
			await loadNext();
			...
			// Snap back so the user can try again.
			dx = 0;
			dy = 0;
			exitDirection = null;
		} finally {
	...
	function onPointerDown(event: PointerEvent) {
		...
		// Don't hijack drags that started on the title link or the
		// action buttons — let those clicks behave normally.
	...
	function onPointerUp(event: PointerEvent) {
		...
		// Horizontal beats vertical (it's the dominant Tinder gesture).
		// Up only wins when the user has clearly committed to up — past
		// threshold AND vertical motion exceeds horizontal.
	...
	function triggerExit(direction: 'left' | 'right' | 'up') {
		...
		// Wait for the CSS exit transition before firing the API call,
		// so the user sees the card fly off rather than disappearing
		// while it's still mid-swipe.
		setTimeout(() => {
	...
	$: cardStyle = (() => {
		...
		// Active drag or snap-back. Rotate slightly so the card feels
		// like it's hinged at the bottom — ~10 degrees at full swing.
	...
	// Decision-direction tint that strengthens with the swipe distance.
			...
			<Card>
				<div class="grid gap-6 md:grid-cols-2">
					...
					<div class="relative aspect-square overflow-hidden rounded-xl bg-slate-100 dark:bg-slate-700">
						...
						<!-- Swipe direction indicators. Fade in with drag distance
						     so the user gets a clear preview of what their gesture
						     will commit to before they release. -->
						{#if dominantDirection === 'right'}
							<div
								class="pointer-events-none absolute inset-0 flex items-start justify-end p-4"
								style="opacity: {dragOpacity}"
							>
								<span class="rotate-12 rounded-md border-4 border-emerald-500 bg-emerald-100/80 px-4 py-2 text-2xl font-extrabold uppercase tracking-wider text-emerald-700">
									Love
								</span>
							</div>
						{:else if dominantDirection === 'left'}
							<div
								class="pointer-events-none absolute inset-0 flex items-start justify-start p-4"
								style="opacity: {dragOpacity}"
							>
								<span class="-rotate-12 rounded-md border-4 border-red-500 bg-red-100/80 px-4 py-2 text-2xl font-extrabold uppercase tracking-wider text-red-700">
									Toss
								</span>
							</div>
						{:else if dominantDirection === 'up'}
							<div
								class="pointer-events-none absolute inset-0 flex items-end justify-center p-4"
								style="opacity: {dragOpacity}"
							>
								<span class="rounded-md border-4 border-amber-500 bg-amber-100/80 px-4 py-2 text-2xl font-extrabold uppercase tracking-wider text-amber-700">
									Maybe
								</span>
							</div>
						{/if}
				...
				<!-- Action buttons (always visible — accessibility + desktop fallback) -->
